...
counties = ['Santa Clara', 'Los Angeles']
...
for i, d in enumerate(x): 
    if d in timeline.keys():
        ax.annotate(str(q), xy=(i, 3000), arrowprops=dict(facecolor='brown', shrink=0.05))
        quotes[i] = timeline[d]
        ax.text(1, 5000 - 200*q, str(q) + ': ' + timeline[d])
        q += 1
ax.text(1, 5000, 'Notes:')
...
plt.title('Santa Clara vs. LA Confirmed COVID-19 Cases (based on NYTimes)')
